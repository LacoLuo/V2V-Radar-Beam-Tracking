...
from model import DA_LSTM, RD_LSTM, RA_LSTM
...
def build_model(config, device):
    if config.feature == "RD":
        model = RD_LSTM()
        summary(model, [(config.batch_size, config.x_size, 1, 256, 128), (config.batch_size, 1)], dtypes=[torch.float, torch.long])
    elif config.feature == "RA":
        model = RA_LSTM()
        summary(model, [(config.batch_size, config.x_size, 1, 256, 64), (config.batch_size, 1)], dtypes=[torch.float, torch.long])
    elif config.feature == "DA":
        model = DA_LSTM()
        summary(model, [(config.batch_size, config.x_size, 1, 128, 64), (config.batch_size, 1)], dtypes=[torch.float, torch.long])
...
def train(config, model, optimizer, train_iter, loss_function, total_steps, device, writer, num_of_train_data):
    ...
    for step in range(config.summary_steps):
        ...
        radar_map, prev_beams, label_beam = next(train_iter)

        radar_map = torch.unsqueeze(radar_map, 2).type(torch.FloatTensor).to(device) 
        prev_beam = prev_beams[:, 0].type(torch.LongTensor).to(device)
        label_beam = label_beam.type(torch.LongTensor).to(device)

        pred_beam = model(radar_map, prev_beam)
...
def inference(config, model, data_loader, device, mode='valid', loss_function=None):
    ...
    y_true = []
    y_pred = []
    for radar_map, prev_beams, label_beam in data_loader:
        radar_map = torch.unsqueeze(radar_map, 2).type(torch.FloatTensor).to(device) 
        prev_beam = prev_beams[:, 0].type(torch.LongTensor).to(device)
        label_beam = label_beam.type(torch.LongTensor).to(device)

        with torch.no_grad():
            pred_beam = model.forward(radar_map, prev_beam)
    ...
    if mode == 'valid':
        ...
        return loss_sum / len(data_loader), top1_acc / len(data_loader), [i / n for i in top5_acc], cm 
...
def train_process(config):
    ...
    train_feed = DataFeed(config.trn_data_path, config.feature, config.len_of_sequence, config.x_size, normalize=config.normalize)
    ...
    val_feed = DataFeed(config.val_data_path, config.feature, config.len_of_sequence, config.x_size, normalize=config.normalize)
    ...
    while (total_steps < config.num_steps):
        ...
        if num_of_epoch != 0 and num_of_epoch % 40 == 0:
        ...
        num_of_epoch = config.batch_size * total_steps // num_of_train_data
        if num_of_epoch >= 60:
            torch.save(model.state_dict(), f'{config.store_model_path}/E2E_{config.x_size}.ckpt')
...
def test_process(config):
    ...
    test_feed = DataFeed(config.test_data_path, config.feature, config.len_of_sequence, config.x_size, normalize=config.normalize)
    ...
    # Test
    _, top1_acc, top5_acc, cm = inference(config, model, test_loader, device, mode='test')
    ...
    figure.savefig(os.path.join(os.path.dirname(config.load_model_path), config.feature+'.png'), dpi=400)
    #figure.savefig(os.path.join(os.path.dirname(config.load_model_path), config.feature+'.eps'), format='eps')
